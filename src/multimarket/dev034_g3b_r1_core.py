...
STATUS_SURVIVOR="G3_LAYER_SURVIVOR"
STATUS_INCONCLUSIVE="G3_LAYER_INCONCLUSIVE"
STATUS_REJECTED="G3_LAYER_REJECTED"

class G3BR1Error(RuntimeError):
...
def metrics(y,p):
    ...
    if len(y)!=len(p) or len(y)==0:
        raise G3BR1Error("metric_length")
    if not np.all(np.isfinite(p)):
        raise G3BR1Error("metric_nonfinite")
...
def _stack(per_day:Mapping[date,tuple[np.ndarray,np.ndarray,np.ndarray]],days:Sequence[date]):
    ...
    if len(t) and np.any(np.diff(t)<=0):
        raise G3BR1Error("stack_chronology")
    if not np.all(np.isfinite(x)):
        raise G3BR1Error("stack_nonfinite")
    if not np.all(np.isin(y,(0,1))):
        raise G3BR1Error("stack_labels")
    return x,y,t
...
def prediction_sha256(cid,fold_id,ts,y,pred,p):
    h=hashlib.sha256(b"DEV034-G3B-R1-PREDICTION-V1\0")
    h.update(str(cid).encode("ascii"))
    h.update(struct.pack(">I",int(fold_id)))
    for t,a,b,q in zip(
...
def fit_candidate(cid,per_day,folds=dd.OUTER_FOLDS):
    ...
    for outer in folds:
        ...
        if feature_count is None:
            ...
            raise G3BR1Error("feature_count_drift",cid)
...
def _aligned(base:CandidateResult,cand:CandidateResult):
    if len(base.folds)!=4 or len(cand.folds)!=4:
        raise G3BR1Error("fold_count")
    for b,c in zip(base.folds,cand.folds,strict=True):
        if b.fold_id!=c.fold_id:
            raise G3BR1Error("fold_id_alignment")
        if not np.array_equal(b.timestamps_us,c.timestamps_us):
            raise G3BR1Error("support_alignment")
        if not np.array_equal(b.labels,c.labels):
            raise G3BR1Error("label_alignment")

def compare(base:CandidateResult,cand:CandidateResult):
    _aligned(base,cand)
    fold_delta=[
        float(c.metrics["balanced_accuracy"]-b.metrics["balanced_accuracy"])
        for b,c in zip(base.folds,cand.folds,strict=True)
    ]
    loo=[]
    for omitted in range(4):
        ...
        loo.append(float(
            balanced_accuracy_score(y,cp)-balanced_accuracy_score(y,bp)
        ))
    total=int(cand.pooled_metrics["support"])
    minority=min(
        int(cand.pooled_metrics["predicted_long_count"]),
        int(cand.pooled_metrics["predicted_short_count"]),
    )/total
    return {
        "pooled_delta_balanced_accuracy":float(
            cand.pooled_metrics["balanced_accuracy"]-base.pooled_metrics["balanced_accuracy"]
        ),
        ...
        "both_classes_predicted_all_folds":bool(all(
            f.metrics["predicted_long_count"]>0 and f.metrics["predicted_short_count"]>0
            for f in cand.folds
        )),
...
def joint_max_stat_null(base:CandidateResult,candidates,*,seed=NULL_SEED,replicates=NULL_REPLICATES):
    ids=tuple(candidates)
    expected=tuple(f"G3C{i:02d}" for i in range(1,17))
    if ids!=expected:
        raise G3BR1Error("null_candidate_order")
    for cid in ids:
        _aligned(base,candidates[cid])
    ...
    for f in base.folds:
        ...
        if len(vals)==0:
            raise G3BR1Error("null_legal_shift_empty",str(f.fold_id))
        legal.append(vals)

    observed={
        cid:float(candidates[cid].pooled_metrics["balanced_accuracy"]-base.pooled_metrics["balanced_accuracy"])
        for cid in ids
    }
