"""DEV030-P8 fixed-lag PRICE temporal-shape diagnostic.

Frozen scientific question:
Does coarse causal path shape from the same three PRICE primitives add stable
T1 LONG_FIRST-vs-SHORT_FIRST information beyond the frozen 32-second PRICE S1
whole-window summaries?

P8 changes representation only. It keeps target A / 120s / 16bp / 32s,
...
EXPERIMENT_ID = "DEV030-P8"
DESIGN_VERSION = "price-temporal-shape-fixed-lags-v1"
...
TEMPORAL_SHAPE_FEATURE_NAMES = tuple(
    f"{feature}__lag_{lag}s"
    for feature in PRICE_LAG_PRIMITIVES
    for lag in LAG_SECONDS
)
AUGMENTED_FEATURE_NAMES = (
    BASELINE_FEATURE_NAMES + TEMPORAL_SHAPE_FEATURE_NAMES
)

EXPECTED_BASELINE_FEATURE_COUNT = 23
EXPECTED_TEMPORAL_SHAPE_FEATURE_COUNT = 12
EXPECTED_AUGMENTED_FEATURE_COUNT = 35
...
ARTIFACT_FILENAME = "DEV030_P8_PRICE_TEMPORAL_SHAPE_RESULT.json"

PREDICTION_HASH_DOMAIN = b"DEV030-P8-OOF-PREDICTION-V1\x00"
...
class P8Error(RuntimeError):
    """Frozen P8 protocol, provenance, model, or output violation."""
...
def _validate_execution_commit(value: str) -> str:
    if (
        ...
        raise P8Error("execution_commit_must_be_full_sha")
    return value
...
def load_verified_json_artifact(
    ...
    *,
    ...
    if not artifact.is_file():
        raise P8Error("frozen_artifact_missing", str(artifact))
    actual = str(hash_file(artifact))
    if actual != expected_sha256:
        raise P8Error("frozen_artifact_sha256_mismatch", str(artifact))
    try:
        payload = json.loads(artifact.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise P8Error("frozen_artifact_read_failed", str(exc)) from exc
    if not isinstance(payload, dict):
        raise P8Error("frozen_artifact_not_object")
    return payload
    ...
    *,
    ...
    for rel, expected_sha, reason in expected:
        ...
        if not file_path.is_file():
            raise P8Error("frozen_dependency_missing", rel)
        actual = str(hash_file(file_path))
        if actual != expected_sha:
            raise P8Error(reason, rel)
        result[rel] = actual
    return result
...
def validate_prior_artifacts(
    ...
    try:
        ...
        raise P8Error(exc.reason, str(exc)) from exc
    expected = (
        (
    ...
    for actual, wanted, reason in expected:
        if actual != wanted:
            raise P8Error(reason)
    if p6_payload.get("eligible_for_direction_capacity_upgrade") is not False:
        raise P8Error("p6_capacity_upgrade_state_mismatch")
    if p7_payload.get("eligible_l1_ofi_incremental_information") is not False:
        raise P8Error("p7_incremental_information_state_mismatch")


def validate_feature_contract() -> None:
    if len(BASELINE_FEATURE_NAMES) != EXPECTED_BASELINE_FEATURE_COUNT:
        raise P8Error("baseline_feature_count_mismatch")
    expected_shape = tuple(
        f"{feature}__lag_{lag}s"
        for feature in PRICE_LAG_PRIMITIVES
        for lag in LAG_SECONDS
    )
    if TEMPORAL_SHAPE_FEATURE_NAMES != expected_shape:
        raise P8Error("temporal_shape_feature_order_mismatch")
    if len(TEMPORAL_SHAPE_FEATURE_NAMES) != EXPECTED_TEMPORAL_SHAPE_FEATURE_COUNT:
        raise P8Error("temporal_shape_feature_count_mismatch")
    if len(AUGMENTED_FEATURE_NAMES) != EXPECTED_AUGMENTED_FEATURE_COUNT:
        raise P8Error("augmented_feature_count_mismatch")
    if len(set(AUGMENTED_FEATURE_NAMES)) != len(AUGMENTED_FEATURE_NAMES):
        raise P8Error("augmented_feature_duplicate")
    if any("__lag_0s" in name for name in TEMPORAL_SHAPE_FEATURE_NAMES):
        raise P8Error("current_value_must_not_be_duplicated")


def validate_candidate(dataset: dd.CandidateDayDataset) -> None:
    validate_feature_contract()
    if dataset.key != SELECTED_KEY:
        raise P8Error("candidate_identity_mismatch")
    if tuple(dataset.s1_feature_names) != BASELINE_FEATURE_NAMES:
        raise P8Error("baseline_feature_order_mismatch")
    ...
    if not (
        ...
        raise P8Error("candidate_array_length_mismatch")
...
def label_sha256(timestamps_us: Any, labels: Any) -> str:
    ...
    if ts.ndim != 1 or y.ndim != 1 or len(ts) != len(y):
        raise P8Error("label_hash_shape_mismatch")
    if not bool(np.all(np.isin(y, (SHORT_FIRST, LONG_FIRST)))):
        raise P8Error("label_hash_invalid_labels")
    if len(ts) and bool(np.any(np.diff(ts) <= 0)):
        raise P8Error("label_hash_timestamps_not_chronological")
    digest = hashlib.sha256()
...
def _exact_positions(
    ...
    if bool(np.any(positions >= len(raw_timestamps_us))):
        raise P8Error("lag_timestamp_missing")
    if not bool(
...
def extract_temporal_shape_matrix(
    sequence_input: sf.SequenceFeatureInput,
    decision_timestamps_us: Any,
) -> np.ndarray:
    """Extract exactly 12 causal PRICE lag landmarks on supplied decisions."""
    ...
    if raw_ts.ndim != 1 or raw_ts.dtype.kind not in "iu":
        raise P8Error("raw_timestamps_must_be_integer_1d")
    if decisions.ndim != 1 or decisions.dtype.kind not in "iu":
        raise P8Error("decision_timestamps_must_be_integer_1d")
    ...
    if len(decisions) and bool(np.any(np.diff(decisions) <= 0)):
        raise P8Error("decision_timestamps_not_chronological")
    if len(raw_ts) == 0 or bool(np.any(np.diff(raw_ts) <= 0)):
        raise P8Error("raw_timestamps_not_chronological")
    if bool(np.any(raw_ts % sf.GRID_US != 0)):
        raise P8Error("raw_timestamp_off_grid")
    ...
    try:
        ...
        raise P8Error("invalid_price_sequence_input") from exc
    ...
    if any(
        ...
        raise P8Error("price_sequence_array_shape_mismatch")
    ...
    for primitive in PRICE_LAG_PRIMITIVES:
        for lag in LAG_SECONDS:
            ...
            if not bool(
                ...
                raise P8Error("lag_snapshot_invalid_required_mask")
            ...
            if primitive == "spread_bps":
                ...
                if bool(np.any(prior_positions < 0)):
                    raise P8Error("lag_prior_mid_missing")
                if not bool(
                    np.array_equal(
                ...
                if not bool(
                    ...
                    raise P8Error("lag_prior_mid_invalid_book")
                ...
                if not bool(
                    ...
                    raise P8Error("lag_mid_invalid")
                values = 10_000.0 * np.log(current_mid / prior_mid)
            else:
                raise P8Error("unexpected_price_primitive")

            if not bool(np.all(np.isfinite(values))):
                raise P8Error("lag_feature_non_finite", primitive)
            columns.append(np.asarray(values, dtype=np.float64))

    if not columns:
        raise P8Error("temporal_shape_columns_empty")
    matrix = np.column_stack(columns)
    if matrix.shape != (
        len(decisions),
        EXPECTED_TEMPORAL_SHAPE_FEATURE_COUNT,
    ):
        raise P8Error("temporal_shape_matrix_shape_mismatch")
    return matrix
...
def build_shape_day(
    ...
    if len(ts) == 0:
        raise P8Error("shape_support_empty")
    if x0.shape != (len(ts), EXPECTED_BASELINE_FEATURE_COUNT):
        raise P8Error("baseline_selected_shape_mismatch")
    if not bool(np.all(np.isfinite(x0))):
        raise P8Error("baseline_features_non_finite")
    if not bool(np.all(np.isin(y, (SHORT_FIRST, LONG_FIRST)))):
        raise P8Error("shape_labels_invalid")

    shape = extract_temporal_shape_matrix(sequence_input, ts)
    x1 = np.column_stack((x0, shape))
    if x1.shape != (len(ts), EXPECTED_AUGMENTED_FEATURE_COUNT):
        raise P8Error("augmented_selected_shape_mismatch")

    # P8 is forbidden from shrinking support relative to frozen PRICE S1.
    if len(x1) != int(np.count_nonzero(mask)):
        raise P8Error("temporal_shape_support_shrink")
...
def reproduce_frozen_p3(
    ...
    if result["pass"] is not True:
        raise P8Error("frozen_p3_reproduction_failed")
    return result
...
def verify_interval_separation(
    ...
    for fold in dd.OUTER_FOLDS:
        ...
        if len(train_ts) == 0 or len(val_ts) == 0:
            raise P8Error("interval_check_support_empty")
        ...
        if train_end >= val_start:
            raise P8Error("training_validation_information_overlap")
        checks.append(
            {
...
def _stack_days(
    per_day: Mapping[date, ShapeDay],
    days: Sequence[date],
    representation: str,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    if representation not in ("C0", "C1"):
        raise P8Error("invalid_representation")
    ...
    if len(ts) and bool(np.any(np.diff(ts) <= 0)):
        raise P8Error("stacked_timestamps_not_chronological")
    return x, y, ts
...
def probability_metrics(y_true: Any, p_long: Any) -> dict[str, Any]:
    ...
    if y.ndim != 1 or p.ndim != 1 or len(y) != len(p) or len(y) == 0:
        raise P8Error("metric_shape_mismatch")
    if not bool(np.all(np.isin(y, (0, 1)))):
        raise P8Error("metric_labels_invalid")
    if len(np.unique(y)) != 2:
        raise P8Error("metric_requires_both_classes")
    if not bool(np.all(np.isfinite(p))) or not bool(
        np.all((p >= 0.0) & (p <= 1.0))
    ):
        raise P8Error("metric_probabilities_invalid")
...
def _new_logistic(c_value: float) -> LogisticRegression:
    if c_value not in C_GRID:
        raise P8Error("c_not_in_frozen_grid")
...
def select_c_probability_first(
    ...
    if xf.ndim != 2 or xv.ndim != 2 or xf.shape[1] != xv.shape[1]:
        raise P8Error("inner_feature_shape_mismatch")
    if len(xf) != len(yf) or len(xv) != len(yv):
        raise P8Error("inner_length_mismatch")
    if len(np.unique(yf)) != 2 or len(np.unique(yv)) != 2:
        raise P8Error("inner_split_requires_both_classes")
    if not bool(np.all(np.isfinite(xf))) or not bool(np.all(np.isfinite(xv))):
        raise P8Error("non_finite_inner_features")
...
def prediction_sha256(
    *,
    ...
    if representation not in ("C0", "C1"):
        raise P8Error("prediction_hash_representation_invalid")
    ts = np.asarray(timestamps_us, dtype=np.int64)
    y = np.asarray(y_true, dtype=np.int8)
    p = np.asarray(p_long, dtype=np.float64)
    if not (ts.ndim == y.ndim == p.ndim == 1):
        raise P8Error("prediction_hash_shape_mismatch")
    if not (len(ts) == len(y) == len(p)):
        raise P8Error("prediction_hash_length_mismatch")
    if len(ts) and bool(np.any(np.diff(ts) <= 0)):
        raise P8Error("prediction_hash_timestamps_not_chronological")
    if not bool(np.all(np.isin(y, (0, 1)))):
        raise P8Error("prediction_hash_labels_invalid")
    if not bool(np.all(np.isfinite(p))) or not bool(
        np.all((p >= 0) & (p <= 1))
    ):
        raise P8Error("prediction_hash_probabilities_invalid")
...
def fit_representation(
    ...
    if tuple(per_day) != dd.HISTORICAL_DAYS:
        raise P8Error("shape_day_order_mismatch")
    folds = tuple(
...
def eligible_shared_shifts(group_sizes: Sequence[int]) -> tuple[int, ...]:
    ...
    if not sizes or any(value <= 0 for value in sizes):
        raise P8Error("invalid_null_group_sizes")
    return tuple(
        k
...
def paired_temporal_null(
    ...
    if len(shifts) < 20:
        raise P8Error("insufficient_temporal_null_shifts")
...
def runtime_provenance(
    *,
    ...
    if type(model_fit_run) is not bool or type(p8_run) is not bool:
        raise P8Error("runtime_flags_must_be_builtin_bool")
    if p8_run and not model_fit_run:
        raise P8Error("p8_requires_model_fit")
    return {
...
def validate_runtime_provenance(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping) or set(value) != required:
        raise P8Error("runtime_provenance_schema_mismatch")
    if value["jan_jul_analytically_opened"] is not True:
        raise P8Error("jan_jul_runtime_state_invalid")
    ...
    if (
        ...
        raise P8Error("forward_data_guard_violation")
    if value["p8_run"] and not value["model_fit_run"]:
        raise P8Error("p8_requires_model_fit")
    prohibited = (
        "threshold_optimization_run",
        "pnl_backtest_run",
    ...
    if any(value[name] is not False for name in prohibited):
        raise P8Error("prohibited_runtime_activity")
    return dict(value)
    ...
    def normalize(value: Any) -> Any:
        ...
        if type(value) is float:
            if not math.isfinite(value):
                raise P8Error("non_finite_json_value")
            return value
        ...
        if isinstance(value, Mapping):
            if not all(isinstance(key, str) for key in value):
                raise P8Error("json_mapping_key_not_string")
            return {
        ...
        raise P8Error("unsupported_json_value", type(value).__name__)
...
def _output_preflight(output_directory: Path) -> None:
    ...
    if output.exists() or output.is_symlink():
        raise P8Error("output_directory_already_exists")
    parent = output.parent
    if not parent.is_dir():
        raise P8Error("output_parent_missing")
    probe = parent / f".{output.name}.preflight"
    if probe.exists() or probe.is_symlink():
        raise P8Error("output_probe_preexists")
    try:
        with probe.open("xb") as handle:
            handle.write(b"DEV030-P8 preflight\n")
            handle.flush()
...
def write_result_once(
    ...
    *,
    ...
    if require_canonical_output and output != REAL_OUTPUT_DIRECTORY:
        raise P8Error("noncanonical_output_directory")
    if not require_canonical_output and output == REAL_OUTPUT_DIRECTORY:
        raise P8Error("canonical_output_requires_real_mode")
    ...
    try:
        ...
        if isinstance(exc, P8Error):
            raise
        raise P8Error("artifact_write_failed", str(exc)) from exc
...
def run_p8(
    *,
    ...
    """Run the separately-authorized canonical P8 development campaign."""
    ...
    if tuple(day.day for day in loaded_days) != dd.HISTORICAL_DAYS:
        raise P8Error("loaded_day_calendar_mismatch")
    ...
    if tuple(candidate_per_day) != dd.HISTORICAL_DAYS:
        raise P8Error("candidate_day_order_mismatch")
    ...
    c0 = fit_representation(shape_days, "C0")
    c1 = fit_representation(shape_days, "C1")
    validate_matched_support(c0, c1)
    ...
    if eligible:
        ...
        status = "FAIL_PRICE_TEMPORAL_SHAPE_TEMPORAL_NULL"
    else:
        status = (
            "FAIL_PRICE_TEMPORAL_SHAPE_NO_STABLE_INCREMENTAL_VALUE"
        )

    runtime = runtime_provenance(model_fit_run=True, p8_run=True)
