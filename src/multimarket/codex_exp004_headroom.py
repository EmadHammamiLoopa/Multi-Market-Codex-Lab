...
from .codex_research import assert_unsealed_day, assert_unsealed_path, canonical_sha256, sha256_file
...
def scheduled_indices(day: DayData, *, horizon_s: int, schedule: str) -> np.ndarray:
    ...
    if schedule not in {"dense_1m", "nonoverlap"}:
...
def executable_fixed_horizon(
    ...
    safe_e = np.minimum(entry, len(day.ts) - 1)
    safe_x = np.minimum(exit_, len(day.ts) - 1)
...
def _distribution(values: np.ndarray) -> dict[str, float | int | None]:
    ...
    if not len(values):
        return {"count": 0, "mean": None, "q50": None, "q75": None, "q90": None, "q95": None, "q99": None}
...
def _eligibility(rows: list[dict[str, Any]], horizon_s: int) -> dict[str, Any]:
    ...
    selected = [row for row in rows if row["schedule"] == "nonoverlap" and row["horizon_s"] == horizon_s]
    if len(selected) != len(SYMBOLS) * len(DAYS):
        raise RuntimeError("incomplete symbol-day matrix")
    key_counts = np.asarray([row["summary"]["headroom"][key]["count"] for row in selected], dtype=np.int64)
    strong_counts = np.asarray([row["summary"]["headroom"][strong]["count"] for row in selected], dtype=np.int64)
    valid_counts = np.asarray([row["summary"]["valid_decisions"] for row in selected], dtype=np.int64)
    total_key = int(key_counts.sum())
    total_valid = int(valid_counts.sum())
    by_symbol = {
        symbol: int(sum(row["summary"]["headroom"][key]["count"] for row in selected if row["symbol"] == symbol))
        for symbol in SYMBOLS
    }
    maximum_share = float(key_counts.max() / total_key) if total_key > 0 else 1.0
    gates = {
        "pooled_24bp_events_at_least_100": total_key >= 100,
        "pooled_24bp_fraction_at_least_1pct": (total_key / total_valid if total_valid else 0.0) >= 0.01,
        "at_least_12_of_14_symbol_days_with_event": int(np.sum(key_counts > 0)) >= 12,
        "median_24bp_events_per_symbol_day_at_least_3": float(np.median(key_counts)) >= 3.0,
        "btc_24bp_events_at_least_30": by_symbol["BTCUSDT"] >= 30,
        "eth_24bp_events_at_least_30": by_symbol["ETHUSDT"] >= 30,
        "max_symbol_day_share_at_most_25pct": maximum_share <= 0.25,
        "pooled_36bp_events_at_least_40": int(strong_counts.sum()) >= 40,
    }
    return {
        "horizon_s": horizon_s,
        "pooled_valid_decisions": total_valid,
        "pooled_24bp_events": total_key,
        "pooled_24bp_fraction": float(total_key / total_valid) if total_valid else 0.0,
...
def input_manifest(feature_dir: Path) -> list[dict[str, Any]]:
    records = []
    for symbol in SYMBOLS:
        for day in DAYS:
...
def run(feature_dir: Path, output: Path, workspace: Path, frozen_commit: str) -> dict[str, Any]:
    ...
    manifest = input_manifest(feature_dir)
        ...
        for day in DAYS:
            ...
            for horizon_s in HORIZONS_S:
                for schedule in ("dense_1m", "nonoverlap"):
                    indices = scheduled_indices(loaded, horizon_s=horizon_s, schedule=schedule)
    ...
    payload = {
        ...
        "interpretation": (
            ...
            "Eligibility authorizes a separately preregistered predictive experiment; it is not a strategy PASS."
        ),
    }
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Model-free CODEX-EXP-004-P0 economic headroom audit")
    ...
    print(json.dumps({
        ...
        "selected_shortest_eligible_horizon_s": result["model_worthiness"]["selected_shortest_eligible_horizon_s"],
    }, indent=2))
    return 0
