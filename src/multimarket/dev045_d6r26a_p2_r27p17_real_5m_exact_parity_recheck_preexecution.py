...
EXPERIMENT_ID = "DEV045-D6R26A-P2-R27P17"
DESIGN_VERSION = "real-5m-exact-parity-recheck-preexecution-v1"
PARENT_R27P16_FREEZE_HEAD = "d71064e5a74b639f88ff0ab0fba3b88bb7578ffe"
AUTH_ENV = "DEV045_D6R26A_P2_R27P17_AUTHORIZE"
AUTH_TOKEN = "YES_REAL_5M_EXACT_PARITY_RECHECK_R10_VOL"
...
class R27P17Error(RuntimeError):
    pass
...
def _require_authorized(environ: dict[str, str] | None = None) -> None:
    if not real_diagnostic_authorized(environ):
        raise R27P17Error("authorization")
...
def diagnose_events(events: np.ndarray, *, prefix_rows: int) -> ParityReport:
    ...
    with TemporaryDirectory(prefix="dev045_r27p17_diag_") as root:
        ...
        try:
            ...
            vol_audit = r27p16.audit_amendment(base, amended)
            if not vol_audit.other_feature_bytes_equal:
                raise R27P17Error("volatility_amendment_scope")
            if any(i not in r27p16.AMENDED_FEATURE_INDICES for i in vol_audit.changed_feature_indices):
                raise R27P17Error("volatility_amendment_feature_indices")
...
def run_real_recheck(*, environ: dict[str, str] | None = None) -> ParityReport:
    ...
    try:
        if bool(events.flags.writeable):
            raise R27P17Error("source_writeable")
    ...
    print(f"R27P17_SOURCE_IDENTITY=PASS DAY={SOURCE_DAY} ROWS={SOURCE_ROWS} BYTES={SOURCE_BYTES} SHA256={SOURCE_SHA256}")
    print(f"R27P17_PREFIX_ROWS={report.prefix_rows}")
    print(f"R27P17_DECISION_COUNT={report.decision_count}")
    print(f"R27P17_LEADING_PREELIGIBLE={report.leading_preeligible_count}")
    print(f"R27P17_L5_AMENDMENT_CHANGED_CELLS={report.l5_changed_cells}")
    print(f"R27P17_VOLATILITY_AMENDMENT_CHANGED_CELLS={report.volatility_changed_cells}")
    print(f"R27P17_MISMATCH_CELLS={report.mismatch_cells}")
    print(f"R27P17_MISMATCH_FEATURE_INDICES={report.mismatch_feature_indices}")
    print(f"R27P17_EXACT_FEATURE_PARITY={'PASS' if report.exact_feature_parity else 'FAIL'}")
    print("R27P17_TIMING_PERFORMED=NO")
    print("R27P17_SPEED_GATE_RUN=NO")
    print("R27P15_RERUN=NO")
    print("100K_RERUN=NO")
    print("1M_RERUN=NO")
    ...
    if not report.exact_feature_parity:
        raise R27P17Error("exact_feature_parity")
    return report


def validate_r27p17_contract() -> None:
    r27p16.validate_r27p16_contract()
    if PARENT_R27P16_FREEZE_HEAD != "d71064e5a74b639f88ff0ab0fba3b88bb7578ffe":
        raise R27P17Error("parent")
    required = (
        PREFIX_ROWS == 5_000_000,
    ...
    if not all(required):
        raise R27P17Error("required_guard")
    forbidden = (
    ...
    if any(forbidden):
        raise R27P17Error("execution_surface_open")
...
def main() -> int:
    try:
        ...
        print(f"R27P17_RESULT=FAIL TYPE={type(exc).__name__} REASON={exc}")
        print("R27P17_SPEED_GATE_RUN=NO")
        print("R27P15_RERUN=NO")
        print("100K_RERUN=NO")
        print("1M_RERUN=NO")
