...
EXPERIMENT_ID = "DEV045-D6R26A-P2-R27P11"
DESIGN_VERSION = "amended-bounded-speed-benchmark-preexecution-v1"
PARENT_R27P10_FREEZE_HEAD = "b8326fe0bc878b2c0889359ca7c401cd39e75a0f"

AUTH_ENV = "DEV045_D6R26A_P2_R27P11_AUTHORIZE"
AUTH_TOKEN = "YES_AMENDED_BOUNDED_SPEED_BENCHMARK_PREATTEMPT"

SOURCE_DAY = r27p10.SOURCE_DAY
SOURCE_PATH = r27p10.SOURCE_PATH
SOURCE_ROWS = r27p10.SOURCE_ROWS
SOURCE_BYTES = r27p10.SOURCE_BYTES
SOURCE_SHA256 = r27p10.SOURCE_SHA256
SOURCE_DAY_START_NS = r27p10.SOURCE_DAY_START_NS
SOURCE_DAY_END_EXCLUSIVE_NS = r27p10.SOURCE_DAY_END_EXCLUSIVE_NS
...
CANDIDATE_IMPLEMENTATION = "R27P6A_PLUS_R27P9_CPYTHON313_SUM_AMENDMENT"
...
class R27P11Error(RuntimeError):
    pass
...
def _require_real_benchmark_authorized(environ: dict[str, str] | None = None) -> None:
    if not real_benchmark_authorized(environ):
        raise R27P11Error("authorization")
...
def build_amended_fused_day_context(
    events: np.ndarray,
    *,
...
) -> tuple[r20.DayContextBuildResult, int]:
    ...
    if requested.size <= 0:
        raise R27P11Error("requested_decision_grid_empty")
    ...
    base = r27p5.run_compiled_feature_kernel(
        surface,
        requested,
        kernel=base_feature_kernel,
    )
    amended = r27p9.run_amended_feature_kernel(
        surface,
        requested,
        base_kernel=base_feature_kernel,
        amendment_kernel=amendment_kernel,
    )
    audit = r27p9.audit_amendment(base, amended)
    if not audit.other_feature_bytes_equal:
        raise R27P11Error("other_feature_bytes_changed")
    if any(index != r27p9.AMENDED_FEATURE_INDEX for index in audit.changed_feature_indices):
        raise R27P11Error("amendment_scope")

    decisions = np.asarray(amended.decision_local_ns, dtype="<i8")
    bids = np.asarray(amended.best_bid_tick, dtype="<i8")
    asks = np.asarray(amended.best_ask_tick, dtype="<i8")
    values = np.asarray(amended.values, dtype=r17.FEATURE_CACHE_VALUE_DTYPE)
    ...
    first_eligible_index = int(amended.leading_preeligible_count)
    if first_eligible_index < 0 or first_eligible_index >= int(requested.size):
        raise R27P11Error("leading_preeligible_count")
    if cache.decision_count != int(requested.size) - first_eligible_index:
        raise R27P11Error("eligible_count")
...
def _build_warmed_candidate_kernels():
    ...
    amendment_kernel = r27p9.build_l5_obi_amendment_kernel()
    fixture = r20.make_synthetic_dual_context_fixture()
    with TemporaryDirectory(prefix="dev045_r27p11_warm_") as root:
        context, _ = build_amended_fused_day_context(
            fixture,
    ...
    return raw_kernel, base_feature_kernel, amendment_kernel
...
def benchmark_loaded_prefix(
    ...
    *,
    ...
    if a.ndim != 1 or a.size <= 0:
        raise R27P11Error("events")
    n = int(prefix_rows)
    if n <= 0 or n > int(a.size) or n > MAX_BENCHMARK_ROWS:
        raise R27P11Error("prefix_rows")
    ...
    try:
        ...
        t0 = perf_counter()
        candidate, changed_cells = build_amended_fused_day_context(
            prefix,
            nominal_day_start_local_ns=int(nominal_day_start_local_ns),
            nominal_day_end_exclusive_local_ns=int(nominal_day_end_exclusive_local_ns),
        ...
        if reference_digest != candidate_digest:
            raise R27P11Error("context_digest")
        if (
            ...
            raise R27P11Error("midpoint_bytes")
        ...
        return FullContextBenchmark(
            ...
            parity=True,
            amendment_changed_cells=int(changed_cells),
...
def run_real_benchmark(*, environ: dict[str, str] | None = None) -> BenchmarkRun:
    ...
    try:
        if bool(events.flags.writeable):
            raise R27P11Error("source_writeable")
        raw_kernel, base_feature_kernel, amendment_kernel = _build_warmed_candidate_kernels()

        with TemporaryDirectory(prefix="dev045_r27p11_real_") as root:
            observations = tuple(
    ...
    print(
        f"R27P11_SOURCE_IDENTITY=PASS ROWS={SOURCE_ROWS} "
        f"BYTES={SOURCE_BYTES} SHA256={SOURCE_SHA256}"
    )
    for item in observations:
        print(
            f"R27P11_PREFIX={item.prefix_rows} "
            f"R20_S={item.reference_seconds:.6f} "
            f"AMENDED_S={item.candidate_seconds:.6f} "
            f"SPEEDUP={item.speedup:.3f} PARITY=PASS "
            f"AMENDMENT_CHANGED_CELLS={item.amendment_changed_cells}"
        )
    print(f"R27P11_GATE_SPEEDUP={gate.speedup:.3f}")
    print(f"R27P11_MIN_REQUIRED_SPEEDUP={MIN_ACCEPTED_SPEEDUP:.1f}")
    print(f"R27P11_FULL_CONTEXT_GATE={'PASS' if gate_pass else 'FAIL'}")
...
def validate_r27p11_contract() -> None:
    r27p10.validate_r27p10_contract()
    r27p9.validate_r27p9_contract()
    if PARENT_R27P10_FREEZE_HEAD != "b8326fe0bc878b2c0889359ca7c401cd39e75a0f":
        raise R27P11Error("parent")
    if (
        SOURCE_DAY != r27p10.SOURCE_DAY
        or SOURCE_PATH != r27p10.SOURCE_PATH
        or SOURCE_ROWS != r27p10.SOURCE_ROWS
        or SOURCE_BYTES != r27p10.SOURCE_BYTES
        or SOURCE_SHA256 != r27p10.SOURCE_SHA256
    ):
        raise R27P11Error("source_identity")
    if PREFIX_ROWS != r27p2.PREFIX_ROWS or GATE_PREFIX_ROWS != PREFIX_ROWS[-1]:
        raise R27P11Error("prefix_plan")
    if MAX_BENCHMARK_ROWS != GATE_PREFIX_ROWS:
        raise R27P11Error("max_rows")
    if MIN_ACCEPTED_SPEEDUP != 10.0:
        raise R27P11Error("speedup_gate")
    if r27p9.REAL_HISTORICAL_BENCHMARK_AUTHORIZED is not False:
        raise R27P11Error("r27p9_historical_surface_changed")
    ...
    if not all(required):
        raise R27P11Error("required_guard")
    ...
    if any(forbidden):
        raise R27P11Error("execution_surface_open")
...
def main() -> int:
    try:
        ...
        print(f"R27P11_RESULT=FAIL TYPE={type(exc).__name__} REASON={exc}")
        print("BENCHMARK_MANIFEST_WRITTEN=NO")
        print("P2_ATTEMPT_CONSUMED=NO")
        print("FULL_JAN_JUL_RERUN=NO")
        print("SIMULATOR_RUN=NO")
...
__all__ = [
    ...
    "PREFIX_ROWS",
    "benchmark_loaded_prefix",
    "build_amended_fused_day_context",
