...
SIGNED_RL2_FEATURES = SIGNED_R_FEATURES + (
    "microprice_minus_mid_bps", "obi_l1", "obi_l5", "obi_l10", "ofi_l1_1s", "ofi_l1_3s",
    "mlofi_l5_1s", "mlofi_l5_3s", "trade_qty_imbalance_1s", "trade_qty_imbalance_3s",
    "trade_count_imbalance_1s", "trade_count_imbalance_3s", "obi_l5_mean_1m",
    "ofi_l1_1s_mean_1m", "trade_qty_imbalance_1s_mean_1m",
)
...
class Config:
    ...
    signed_rl2_features: tuple[str, ...] = SIGNED_RL2_FEATURES
    model_c: float = 1.0
    solver: str = "lbfgs"
    class_weight: str | None = None
    max_iter: int = 1000
    seed: int = SEED
    pooled_auc_min: float = 0.60
    pooled_ap_prevalence_multiple_min: float = 1.30
    pooled_top_decile_lift_min: float = 1.50
    symbol_auc_min: float = 0.57
    symbol_top_decile_lift_min: float = 1.25
    nonoverlap_auc_min: float = 0.57
    nonoverlap_top_decile_lift_min: float = 1.25
    rl2_auc_delta_min: float = 0.01
    rl2_ap_delta_min: float = 0.01
    real_auc_minus_placebo_min: float = 0.03
    canary_auc_minus_real_min: float = 0.10
...
class FixedLogistic:
    ...
    def fit(self, X, y):
        ...
        if X.ndim != 2 or len(X) < 2 or len(X) != len(y) or np.unique(y).size != 2:
            raise RuntimeError("invalid training matrix/labels")
...
def build_day_dataset(symbol, day):
    if symbol not in SYMBOLS or day.day not in DAYS: raise ValueError("symbol/day outside frozen P1")
...
def _concat_r_on_rl2_support(days):
    xs = []; ys = []
    for d in days:
        m = d.valid_RL2; xs.append(d.X_R[m]); ys.append(d.y[m])
...
def _cal(y, p):
    ...
    c = np.clip(p, 1e-6, 1 - 1e-6); z = np.log(c / (1 - c)).reshape(-1, 1); m = LogisticRegression(C=1e6, solver="lbfgs", max_iter=1000).fit(z, y)
...
def run(feature_dir, output, workspace, frozen_commit):
    ...
    for outer_day in OUTER_DAYS:
        ...
        for symbol in SYMBOLS:
            ...
            XR, yR, mag = _concat(train, "R"); XL, yL, _ = _concat(train, "RL2"); XRC, yRC = _concat_r_on_rl2_support(train)
            r = FixedLogistic().fit(XR, yR); l = FixedLogistic().fit(XL, yL); rc = FixedLogistic().fit(XRC, yRC)
            vi = R_FEATURE_NAMES.index("rv_30m_bps"); vol = FixedLogistic().fit(XR[:, [vi]], yR); plac = FixedLogistic().fit(XR, _permuted(train)); can = FixedLogistic().fit(np.column_stack((XR, mag)), yR)
            ...
            XRO = outer.X_R[rm]; XLO = outer.X_RL2[lm]; probr = r.predict_proba(XRO); probv = vol.predict_proba(XRO[:, [vi]]); probp = plac.predict_proba(XRO); probc = can.predict_proba(np.column_stack((XRO, outer.oracle_gross_bps[rm])))
            signed_r = [R_FEATURE_NAMES.index(n) for n in SIGNED_R_FEATURES]; signR = XRO.copy(); signR[:, signed_r] *= -1; probs = r.predict_proba(signR)
            probl = l.predict_proba(XLO); probrc = rc.predict_proba(outer.X_R[lm]); signed_l = [RL2_FEATURE_NAMES.index(n) for n in SIGNED_RL2_FEATURES]; signL = XLO.copy(); signL[:, signed_l] *= -1; probsl = l.predict_proba(signL)
            train_counts.append({"outer_day": outer_day.isoformat(), "symbol": symbol, "R_train_n": int(len(yR)), "RL2_train_n": int(len(yL)), "R_common_train_n": int(len(yRC)), "R_outer_n": int(rm.sum()), "RL2_outer_n": int(lm.sum())})
            for j, idx in enumerate(ridx.tolist()):
                rec = {"outer_day": outer_day.isoformat(), "symbol": symbol, "timestamp_us": int(outer.timestamp_us[idx]), "label": int(outer.y[idx]), "oracle_gross_bps": float(outer.oracle_gross_bps[idx]), "nonoverlap_10m": bool(outer.nonoverlap_10m[idx]), "p_R": float(probr[j]), "p_VOL": float(probv[j]), "p_PLACEBO_R": float(probp[j]), "p_CANARY_R": float(probc[j]), "p_SIGN_R": float(probs[j]), "p_RL2": None, "p_R_COMMON_RL2": None, "p_SIGN_RL2": None}
                if idx in lookup:
                    q = lookup[idx]; rec["p_RL2"] = float(probl[q]); rec["p_R_COMMON_RL2"] = float(probrc[q]); rec["p_SIGN_RL2"] = float(probsl[q])
                records.append(rec)
    M = {k: metrics(records, v) for k, v in {"R": "p_R", "VOL": "p_VOL", "PLACEBO_R": "p_PLACEBO_R", "CANARY_R": "p_CANARY_R", "SIGN_R": "p_SIGN_R"}.items()}
    common = [r for r in records if r["p_RL2"] is not None]; M["R_COMMON_RL2"] = metrics(common, "p_R_COMMON_RL2"); M["RL2"] = metrics(common, "p_RL2"); M["SIGN_RL2"] = metrics(common, "p_SIGN_RL2")
    rg = gates(M["R"]); lg = gates(M["RL2"]); rp = M["R"]["pooled"]; rc = M["R_COMMON_RL2"]["pooled"]; lp = M["RL2"]["pooled"]; pp = M["PLACEBO_R"]["pooled"]; cp = M["CANARY_R"]["pooled"]; sr = M["SIGN_R"]["pooled"]; sl = M["SIGN_RL2"]["pooled"]
