...
EXPERIMENT_ID = "CODEX-EXP-019-P1"
...
class Config:
    ...
    exp018_invalid_result_sha256: str = EXP018_RESULT_SHA256
    authorized_aug_path: str = str(AUTHORIZED_AUG_PATH)
    codex_research_parent_blob_sha: str = CODEX_RESEARCH_PARENT_BLOB_SHA
...
def build_validation_dataset(day: Any) -> ValidationDataset:
    if day.day != VALIDATION_DAY:
        raise ValueError("wrong EXP019 validation day")
...
def opaque_sha256_exact_authorized_aug(path: Path) -> str:
    resolved = path.resolve(strict=True)
    expected = AUTHORIZED_AUG_PATH.resolve(strict=True)
    if resolved != expected:
        raise RuntimeError(
            f"unauthorized Aug validation path: {resolved}"
        )
...
def verify_exp018_invalid_lineage(workspace: Path) -> dict[str, Any]:
    p = workspace / EXP018_RESULT
    digest = sha256_file(p)
    if digest != EXP018_RESULT_SHA256:
        raise RuntimeError("EXP018 invalid artifact SHA mismatch")
    ...
    for k, v in required.items():
        if a.get(k) != v:
            raise RuntimeError(f"EXP018 invalid lineage mismatch: {k}")
    return a


def verify_global_seal_source_unchanged(workspace: Path) -> str:
    actual = _git(
        workspace,
        "hash-object",
        "src/multimarket/codex_research.py",
    )
    if actual != CODEX_RESEARCH_PARENT_BLOB_SHA:
        raise RuntimeError("global research seal source changed")
    return actual
...
def run(
    ...
    if output.exists() or output.with_name(output.name + ".part").exists():
        raise RuntimeError("EXP019 output already exists")

    verify_exp018_invalid_lineage(workspace)
    research_blob_sha = verify_global_seal_source_unchanged(workspace)
    ...
    aug_sha = opaque_sha256_exact_authorized_aug(aug_feature)
    ...
    invariants = {
        "exp018_invalid_artifact_sha_exact":
            sha256_file(workspace / EXP018_RESULT) == EXP018_RESULT_SHA256,
        "exp018_invalid_state_proves_aug_unopened":
            True,
        "global_research_seal_source_unchanged":
            research_blob_sha == CODEX_RESEARCH_PARENT_BLOB_SHA,
        "authorized_aug_path_exact":
            aug_feature.resolve(strict=True)
            == AUTHORIZED_AUG_PATH.resolve(strict=True),
    ...
    payload = {
        ...
        "exp018_invalid_result_sha256": EXP018_RESULT_SHA256,
        "codex_research_blob_sha": research_blob_sha,
        "authorized_aug_path": str(AUTHORIZED_AUG_PATH),
        ...
        "interpretation": (
            "Corrected independent Aug-01 opportunity-occurrence validation "
