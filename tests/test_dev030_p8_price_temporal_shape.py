...
def _reason(function: Any, *args: Any, **kwargs: Any) -> str:
    with pytest.raises(p8.P8Error) as caught:
...
def _candidate_day(
    ...
    names = p8.BASELINE_FEATURE_NAMES
    s1 = np.column_stack([
...
def _shape_day(day: date, rows: int = 96) -> p8.ShapeDay:
    ts = (
        day.month * 10**12
    ...
    x0 = np.column_stack([
        ...
        for j in range(p8.EXPECTED_BASELINE_FEATURE_COUNT)
    ])
    shape = np.column_stack([
        np.cos(np.arange(rows) * (0.02 + j * 0.0009))
        + (0.04 + (j % 3) * 0.005) * labels
        for j in range(p8.EXPECTED_TEMPORAL_SHAPE_FEATURE_COUNT)
    ])
    ...
    return p8.ShapeDay(
        ...
        c0_values=x0,
        c1_values=x1,
        c0_feature_names=p8.BASELINE_FEATURE_NAMES,
        c1_feature_names=p8.AUGMENTED_FEATURE_NAMES,
        support_sha256=dd.support_sha256(ts),
        label_sha256=p8.label_sha256(ts, labels),
    )


def _per_day(rows: int = 96) -> dict[date, p8.ShapeDay]:
    return {day: _shape_day(day, rows=rows) for day in dd.HISTORICAL_DAYS}


def test_feature_contract_exact_counts() -> None:
    p8.validate_feature_contract()
    assert len(p8.BASELINE_FEATURE_NAMES) == 23
    assert len(p8.TEMPORAL_SHAPE_FEATURE_NAMES) == 12
    assert len(p8.AUGMENTED_FEATURE_NAMES) == 35


def test_lag_family_is_exact_and_has_no_current_duplicate() -> None:
    assert p8.LAG_SECONDS == (32, 24, 16, 8)
    assert p8.PRICE_LAG_PRIMITIVES == (
        "spread_bps",
        "microprice_minus_mid_bps",
        sf.DERIVED_MID_RETURN,
    )
    assert all("__lag_0s" not in name for name in p8.TEMPORAL_SHAPE_FEATURE_NAMES)


def test_temporal_shape_feature_order_is_primitive_then_lag() -> None:
    assert p8.TEMPORAL_SHAPE_FEATURE_NAMES[:4] == (
        "spread_bps__lag_32s",
        "spread_bps__lag_24s",
        "spread_bps__lag_16s",
        "spread_bps__lag_8s",
    ...
    assert p8.TEMPORAL_SHAPE_FEATURE_NAMES[-4:] == (
        ...
        "mid_log_return_250ms_bps__lag_24s",
        "mid_log_return_250ms_bps__lag_16s",
        "mid_log_return_250ms_bps__lag_8s",
    )


def test_extract_temporal_shape_matrix_exact_values() -> None:
    inp = _sequence_input(seconds=100)
    decisions = np.array([40_000_000, 60_000_000], dtype=np.int64)
    matrix = p8.extract_temporal_shape_matrix(inp, decisions)
    assert matrix.shape == (2, 12)

    ts = np.asarray(inp.timestamps_us)
    spread = np.asarray(inp.features["spread_bps"])
    expected = []
    for lag in p8.LAG_SECONDS:
        idx = int(np.where(ts == decisions[0] - lag * 1_000_000)[0][0])
        expected.append(spread[idx])
    assert np.allclose(matrix[0, :4], expected)
...
def test_extract_temporal_shape_rejects_missing_lag_timestamp() -> None:
    inp = _sequence_input(seconds=100)
    ...
    assert _reason(
        p8.extract_temporal_shape_matrix,
        broken,
        np.array([40_000_000], dtype=np.int64),
    ) == "lag_timestamp_missing"


def test_extract_temporal_shape_rejects_invalid_lag_mask() -> None:
    inp = _sequence_input(seconds=100)
...
def test_build_shape_day_preserves_candidate_support() -> None:
    ...
    shape = p8.build_shape_day(candidate, inp)
    assert np.array_equal(shape.timestamps_us, decisions)
    assert shape.c0_values.shape == (2, 23)
    assert shape.c1_values.shape == (2, 35)
...
def test_select_c_probability_first_returns_frozen_c() -> None:
    ...
    c, ledger = p8.select_c_probability_first(xf, yf, xv, yv)
    assert c in p8.C_GRID
    assert [row["C"] for row in ledger] == list(p8.C_GRID)
    assert all("binary_log_loss" in row for row in ledger)


def test_logistic_family_has_no_class_weight() -> None:
    model = p8._new_logistic(0.1)
...
def test_fit_representation_has_four_outer_folds() -> None:
    result = p8.fit_representation(_per_day(), "C0")
...
def test_label_hash_changes_when_label_changes() -> None:
    ...
    assert p8.label_sha256(ts, y1) != p8.label_sha256(ts, y2)


def test_matched_support_rejects_timestamp_change() -> None:
    per_day = _per_day()
    c0 = p8.fit_representation(per_day, "C0")
    c1 = p8.fit_representation(per_day, "C1")
    first = c1.folds[0]
    changed = p8.FoldResult(
        **{
            **first.__dict__,
...
def test_expected_support_validator_rejects_synthetic_support() -> None:
    result = p8.fit_representation(_per_day(), "C0")
    assert _reason(
        p8.validate_expected_p3_support,
        result,
...
def test_comparison_has_required_gates() -> None:
    ...
    c0 = p8.fit_representation(per_day, "C0")
    c1 = p8.fit_representation(per_day, "C1")
    result = p8.comparison_summary(c0, c1, invariants_pass=True)
...
def test_common_shift_rule_matches_p3_support_minimum() -> None:
    assert p8.eligible_shared_shifts([159, 64, 126, 224]) == tuple(range(10, 55))


def test_missing_null_cannot_promote() -> None:
    gates = p8.final_gates({"precheck_gates": {"x": True}}, None)
...
def test_runtime_provenance_prohibits_search_and_forward() -> None:
    state = p8.runtime_provenance(model_fit_run=True, p8_run=True)
...
def test_runtime_validator_rejects_lag_search() -> None:
    ...
    assert _reason(
        p8.validate_runtime_provenance,
        state,
...
def test_canonical_json_rejects_integer_mapping_key() -> None:
    assert _reason(
        p8.canonical_json_bytes,
        {"x": {1: "bad"}},
    ) == "json_mapping_key_not_string"


def test_canonical_json_is_deterministic() -> None:
    assert p8.canonical_json_bytes({"b": 2, "a": 1}) == p8.canonical_json_bytes(
        {"a": 1, "b": 2}
    )
