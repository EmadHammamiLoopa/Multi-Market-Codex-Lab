...
def synthetic_day(rows: int = 20000) -> DayData:
    ...
    return DayData(day, ts, bid, ask, mid, valid, {"L0": valid.copy(), "L1": valid.copy(), "L2": valid.copy()}, {"L0": np.empty((rows, 0)), "L1": np.empty((rows, 0)), "L2": np.empty((rows, 0))})
...
class HeadroomAuditTests(unittest.TestCase):
    ...
    def test_touch_semantics_and_oracle(self) -> None:
        ...
        self.assertAlmostEqual(result["oracle_gross_bps"][0], max(expected_long, expected_short))
    ...
    def test_invalid_entry_or_exit_book_is_rejected(self) -> None:
        day = synthetic_day()
    ...
    def test_threshold_counts_are_deterministic(self) -> None:
        ...
        self.assertEqual(summary["headroom"][str(int(KEY_HEADROOM_BPS))]["count"], 3)
        self.assertEqual(summary["headroom"][str(int(STRONG_HEADROOM_BPS))]["count"], 2)
        self.assertAlmostEqual(summary["headroom"][str(int(KEY_HEADROOM_BPS))]["fraction"], 0.75)
            ...
            for symbol in ("BTCUSDT", "ETHUSDT"):
                for day in DAYS:
                    for schedule in ("dense_1m", "nonoverlap"):
                        key_count = 10 if (schedule == "nonoverlap" and horizon_s in (300, 600)) else 0
                        strong_count = 5 if (schedule == "nonoverlap" and horizon_s in (300, 600)) else 0
                        summary = {
                            "valid_decisions": 100,
                            "headroom": {
                                str(int(KEY_HEADROOM_BPS)): {"count": key_count, "fraction": key_count / 100},
                                str(int(STRONG_HEADROOM_BPS)): {"count": strong_count, "fraction": strong_count / 100},
                            },
                        }
                        rows.append({"symbol": symbol, "day": day.isoformat(), "horizon_s": horizon_s, "schedule": schedule, "summary": summary})
