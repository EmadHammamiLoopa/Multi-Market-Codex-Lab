...
from multimarket import dev045_d6r19_canonical_runner as r
...
def configure_result_surface(monkeypatch, tmp_path: Path) -> Path:
    root = tmp_path / "dev045_d6r19_canonical_economic_run_v1"
    monkeypatch.setattr(r, "RESULT_ROOT", root)
    monkeypatch.setattr(
        r,
        "FINAL_RESULT_PATH",
        root / "DEV045_D6R19_CANONICAL_ECONOMIC_RESULT.json",
    )
    monkeypatch.setattr(
        r,
        "FAILURE_RESULT_PATH",
        root / "DEV045_D6R19_CANONICAL_FAILURE.json",
    )
    return root
...
def test_execution_is_closed_by_default():
    assert r.EXPERIMENT_ID == "DEV045-D6R19"
    ...
    assert r.RESPONSE_BATCH_COHERENT_REPLACEMENT is True
...
def test_exact_d6r19_authorization_gate(monkeypatch):
    assert r.AUTHORIZATION_ENV == "DEV045_D6R19_AUTHORIZE"
...
def test_exact_112_replay_order_and_boundary(monkeypatch):
    ...
    monkeypatch.setattr(
        r, "validate_canonical_verified_source", lambda source, *, day: None
    )
...
def test_runner_contract_rejects_d6r18_and_d6r17_kernel_bindings(monkeypatch):
    ...
    with pytest.raises(r.CanonicalRunnerError, match="d6r19_kernel_binding"):
        r.validate_runner_contract()
...
def test_d6r19_kernel_captures_raw_fills_and_reaccounts_before_close(
    monkeypatch,
):
    lifecycle = []
    ...
    class FakeD6R19Kernel:
        def __init__(self, **kwargs):
            lifecycle.append("d6r19_kernel_init")
    ...
    class ForbiddenKernel:
        def __init__(self, **kwargs):
            raise AssertionError("prior kernel instantiated")

    monkeypatch.setattr(r, "HISTORICAL_KERNEL", FakeD6R19Kernel)
    monkeypatch.setattr(
        r.d6r18_driver,
    ...
    monkeypatch.setattr(
        r,
        "_hftbacktest_module",
        lambda: SimpleNamespace(HashMapMarketDepthBacktest=lambda assets: bt),
    )
    monkeypatch.setattr(
        r.base,
...
def test_raw_fill_event_and_count_parity_are_required():
    replay = make_replay(
        policy_id="M01",
        day="2026-01-01",
        scenario="Q0_PRIMARY_250_250",
    ...
    event = SimpleNamespace(kind=binding.FILL, fill=None)
    with pytest.raises(r.CanonicalRunnerError, match="fill_event_without_record"):
        r.extract_fill_records((event,))


def test_m6_reaccount_cycle_parity_is_required(monkeypatch):
    fill = m6.FillRecord(
        policy_id="M01",
        day="2026-01-01",
...
def test_source_is_opened_once_for_all_16_day_replays(monkeypatch):
    authorize(monkeypatch)
    lifecycle = []
...
def test_frozen_direct_action_support_semantics_are_unchanged(monkeypatch):
    calls = []
    ...
    assert calls == [("2026-04-01", "M06"), ("2026-04-01", "M07")]
...
def test_june_authoritative_source_identity_is_exact_without_opening_source():
    ...
    assert spec == r.frozen._spec_for_day("2026-06-01")


@pytest.mark.parametrize(
    ("replay_kwargs", "message"),
    (
        ({"integrity_failures": 1}, "execution_integrity_failure"),
        ({"terminal_flat": False}, "terminal_not_flat"),
        ({"working_slots": 1}, "terminal_working_quotes"),
    ),
)
def test_execution_integrity_failure_stops_matrix(
    monkeypatch,
    replay_kwargs,
    message,
):
...
def test_source_identity_change_stops_matrix(monkeypatch):
    authorize(monkeypatch)
    source = SimpleNamespace(path=Path("/synthetic/source.npy"))
    ...
    monkeypatch.setattr(r.bridge, "_validate_day_mapping", lambda **kwargs: None)
    monkeypatch.setattr(r.base, "_stat_identity", lambda path: next(identities))
    monkeypatch.setattr(
        r,
        "run_bound_verified_replay_with_fills",
        lambda source, *, policy_id, day, scenario, direct_index: make_captured(
            policy_id=policy_id,
            day=day,
            scenario=scenario,
        ),
    ...
    with pytest.raises(r.CanonicalRunnerError, match="source_identity_changed"):
        r.run_verified_day_matrix_with_boundary(
            source,
            day="2026-01-01",
            direct_by_policy={},
...
def test_negative_economics_do_not_stop_the_16_replay_matrix(monkeypatch):
    authorize(monkeypatch)
    calls = []
    source = SimpleNamespace(path=Path("/synthetic/source.npy"))
...
@pytest.mark.parametrize(
    "failure_stage",
    ("support", "source_sha", "open", "validation", "source_stat"),
)
def test_pre_first_replay_failure_does_not_consume_attempt(
    ...
    if failure_stage in ("source_sha", "open"):
        monkeypatch.setattr(
            r,
            "open_verified_day_source",
            lambda **kwargs: (_ for _ in ()).throw(
                RuntimeError(f"{failure_stage}_fail")
            ),
        )
    else:
        monkeypatch.setattr(r, "open_verified_day_source", lambda **kwargs: Source())
    ...
    if failure_stage == "validation":
        monkeypatch.setattr(
            ...
            lambda source, *, day: (_ for _ in ()).throw(
                RuntimeError("validation_fail")
            ),
        )
    else:
        monkeypatch.setattr(
...
def test_authorization_failure_does_not_consume_attempt(monkeypatch, tmp_path):
    configure_result_surface(monkeypatch, tmp_path)
    monkeypatch.delenv(r.AUTHORIZATION_ENV, raising=False)
...
def test_first_replay_entry_consumes_and_post_boundary_failure_is_recorded(
    monkeypatch,
    tmp_path,
):
    authorize(monkeypatch)
    ...
    monkeypatch.setattr(
        r, "run_canonical_day_from_disk_with_boundary", fail_first_replay
    )
    ...
    assert payload["experiment_id"] == "DEV045-D6R19"
    ...
    assert payload["response_batch_coherent_replacement"] is True
    assert payload["fresh_replacement_semantics"] is True
    assert r.FINAL_RESULT_PATH.exists() is False


def test_final_arena_is_unreachable_before_all_112(monkeypatch, tmp_path):
    authorize(monkeypatch)
    configure_result_surface(monkeypatch, tmp_path)
    completed = 0
    ...
    monkeypatch.setattr(r, "run_canonical_day_from_disk_with_boundary", day_runner)
...
def test_final_arena_called_exactly_once_after_complete_112(
    monkeypatch,
    tmp_path,
):
    authorize(monkeypatch)
    ...
    def day_runner(**kwargs):
        kwargs["mark_attempt_started"]()
        result = make_day(kwargs["day"])
        events.extend(
            (item.replay.day, item.replay.policy_id, item.replay.scenario)
...
def test_d6r19_result_surface_isolated_from_consumed_attempts():
    assert r.RESULT_ROOT.name == "dev045_d6r19_canonical_economic_run_v1"
    assert r.FINAL_RESULT_PATH.name == (
        "DEV045_D6R19_CANONICAL_ECONOMIC_RESULT.json"
    )
    assert r.FAILURE_RESULT_PATH.name == "DEV045_D6R19_CANONICAL_FAILURE.json"
    assert r.day_evidence_path("2026-04-01").name == (
        "2026-04-01_DEV045_D6R19_DAY_RESULT.json"
    )
    paths = " ".join(
        (str(r.RESULT_ROOT), str(r.FINAL_RESULT_PATH), str(r.FAILURE_RESULT_PATH))
    )
    assert "d6r17" not in paths.lower()
    assert "d6r18" not in paths.lower()
...
def test_frozen_d6r19_driver_and_d6r18_runner_blobs_are_exact():
    assert git_blob(Path(coherent.__file__)) == r.D6R19_DRIVER_BLOB == (
        "5aaf880795f796d5086b3b86a6e7dde0fd7f56ed"
    )
    assert git_blob(Path(r.frozen.__file__)) == r.D6R18_RUNNER_BLOB == (
        "f8d81680ef75a3095f57bada6e01664391e69b72"
    )


def test_dedicated_workflow_is_synthetic_only_and_has_no_authorization():
    workflow = Path(
        ".github/workflows/dev045_d6r19_canonical_runner.yml"
    ).read_text(encoding="utf-8")
    assert "tests/test_dev045_d6r19_canonical_runner.py" in workflow
    assert "tests/test_dev045_d6r19_response_batch_replacement_driver.py" in workflow
    ...
    assert r.AUTHORIZATION_ENV not in workflow
    assert "/home/emadh/Multi-Market/evidence" not in workflow
    assert "CANONICAL_DATA_OPENED_IN_CI=NO" in workflow
    assert "HISTORICAL_ECONOMICS_IN_CI=NO" in workflow


def test_tests_use_only_mock_or_synthetic_sources():
    text = Path(__file__).read_text(encoding="utf-8")
    assert "/synthetic/source.npy" in text
    for forbidden in (
        "_open_verified_" + "file(",
        "np." + "load(",
        "numpy." + "load(",
        "hashlib.sha256(" + "path.read_bytes())",
        "run_bound_verified_replay_with_fills(" + "source=",
    ):
