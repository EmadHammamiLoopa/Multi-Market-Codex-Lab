...
from scripts.codex_exp013_p0_corrected_expiry_segmented_options_flow_readiness import (
    ATM_LOG_MONEYNESS,
    DATES,
    GRID_COUNT,
    GRID_END_MINUTE,
...
class Exp013P0Tests(unittest.TestCase):
    ...
    def test_deribit_symbol_expiry_is_0800_utc(self):
        meta = parse_symbol("BTC-27MAR26-100000-C")
        self.assertIsNotNone(meta)
        expiry = datetime.fromtimestamp(meta[1] / 1_000_000, tz=timezone.utc)
        self.assertEqual(expiry.isoformat(), "2026-03-27T08:00:00+00:00")

    def test_expiry_boundary_uses_corrected_0800_timestamp(self):
        meta = parse_symbol("BTC-27MAR26-100000-C")
        expiry_us = meta[1]
        self.assertEqual(classify_maturity(expiry_us, expiry_us - 1), "short")
        self.assertEqual(classify_maturity(expiry_us, expiry_us), "invalid_expired")
